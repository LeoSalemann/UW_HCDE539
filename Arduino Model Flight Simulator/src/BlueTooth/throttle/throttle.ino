...
void setup(void)
{
  ...
  /**** MAYBE THIS PART SHOULD BE UNCOMMENTED EVEN FOR GAME PADS ****/
...
// AHA! https://learn.adafruit.com/introducing-adafruit-ble-bluetooth-low-energy-friend/ble-services
  /* Enable HID Service (including Mouse) */
  Serial.println(F("Enable HID Service (including Mouse): "));
  if (! ble.sendCommandCheckOK(F( "AT+BLEHIDGAMEPADEN=On"  ))) {
    error(F("Failed to enable HID (firmware >=0.6.6?)"));
  } ;
...
void loop(void)
{
    ...
    ble.println(F("AT+BLEHIDGAMEPAD=0,-1,0x00"));
    ble.println(F("AT+BLEHIDGAMEPAD=0,0,0x00"));
    ble.println(F("AT+BLEHIDGAMEPAD=0,1,0x00"));
